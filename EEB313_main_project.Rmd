...
# define parameters
beta_0 <- 0.6  # Unvaccinated transmission rate
beta_A <- 0.05  # Vaccinated A transmission rate
beta_B <- 0.09  # Vaccinated B transmission rate 0.6
beta_AB <- 0.001 # Transmission rate of individual with AB vaccine

gamma_0 <- 0.01 # Unvaccinated recovery rate 0.15
gamma_A <- 0.5 # Recovery rate for vaccine A
gamma_B <- 0.2   # Recovery rate for vaccine B
gamma_AB <- 0.7  # Recovery rate for both vaccines

c <- 0.5  # Rate of culling

V_A <- 0.4 # vaccine rate of vaccine A
V_B <- 0.4 # vaccine rate of vaccine B
V_AB <- 0.2 # rate of getting vaccinated by both A and B
...
model <- function(time, state, params) {
  with(as.list(c(state, params)), {
    # Model equations (similar to your original code)
...
simulate_model <- function(params, times, state) {
  ode_result <- ode(y = state, times = times, func = model, parms = params)
  return(ode_result)
}

# Function to extract maximum infected individuals for each strategy
get_max_infected <- function(simulation_result) {
  max_infected <- apply(simulation_result[, grep("^I", colnames(simulation_result))], 1, max)
  return(max_infected)
}

# Define strategies
strategies <- list(
  no_culling_no_vaccine = params,
  culling_no_vaccine = params,
  vaccine_A_no_culling = params,
  vaccine_A_culling = params,
  vaccine_B_no_culling = params,
  vaccine_B_culling = params,
  vaccine_AB_no_culling = params,
  vaccine_AB_culling = params
)

# Modify parameters for different strategies
strategies$culling_no_vaccine["c"] <- 1
strategies$vaccine_A_no_culling["V_A"] <- 0.4
strategies$vaccine_A_culling[c("V_A", "c")] <- c(0.4, 1)
strategies$vaccine_B_no_culling["V_B"] <- 0.4
strategies$vaccine_B_culling[c("V_B", "c")] <- c(0.4, 1)
strategies$vaccine_AB_no_culling[c("V_A", "V_B")] <- c(0.2, 0.2)
strategies$vaccine_AB_culling[c("V_A", "V_B", "c")] <- c(0.2, 0.2, 1)

# Create a data frame to store results
results <- data.frame()

# Simulate and extract maximum infected for each strategy
for (strategy_name in names(strategies)) {
  simulation_result <- simulate_model(strategies[[strategy_name]], times, state)
  max_infected <- get_max_infected(simulation_result)
  results <- rbind(results, data.frame(Time = simulation_result[, "time"], Max_Infected = max_infected, Strategy = strategy_name))
}

# Plot the results using ggplot2
ggplot(results, aes(x = Time, y = Max_Infected, color = Strategy)) +
  geom_line() +
  labs(title = "Maximum Number of Infected Individuals Over Time",
       x = "Time",
       y = "Max Infected Individuals") +
  theme_minimal() +
...
# Plot everything
out %>%
  select(time, I, I_A, I_B, I_AB) %>%
  pivot_longer(!time) %>%
  ggplot(aes(x = time, y = value, color = name)) +
  geom_line() +
  ggtitle("Overall Infection")
```
...
# Convert the data to long format for easier plotting with ggplot
out_long_I <- out %>%
  pivot_longer(cols = c("I", "I_A", "I_B", "I_AB"), names_to = "Group_I", values_to = "Infected")

# Plot for every possible scenario using facets
ggplot(out_long_I, aes(x = time, y = Infected, color = Group_I)) +
  geom_line() +
  labs(title = "Disease Spread with Vaccination and Culling",
       x = "Time",
       y = "Infected Individuals") +
  scale_color_manual(values = c("purple", "red", "blue", "green")) +
  facet_wrap(~ Group_I, scales = "free_y") +
  theme_minimal()
...
# Convert the data to long format for easier plotting with ggplot
out_long_S <- out %>%
  pivot_longer(cols = c("S", "S_A", "S_B", "S_AB"), names_to = "Group_S", values_to = "Susceptible")

# Plot for every possible scenario using facets
ggplot(out_long_S, aes(x = time, y = Susceptible, color = Group_S)) +
  geom_line() +
  labs(title = "Disease Spread with Vaccination and Culling",
       x = "Time",
       y = "Susceptible Individuals") +
  scale_color_manual(values = c("purple", "red", "blue", "green")) +
  facet_wrap(~ Group_S, scales = "free_y") +
  theme_minimal()
```

```{r}
out_long_R <- out %>%
  pivot_longer(cols = c("R", "R_A", "R_B", "R_AB"), names_to = "Group_R", values_to = "Recovered")

# Plot for every possible scenario using facets
ggplot(out_long_R, aes(x = time, y = Recovered, color = Group_R)) +
  geom_line() +
  labs(title = "Disease Spread with Vaccination and Culling",
       x = "Time",
       y = "Recovered Individuals") +
  scale_color_manual(values = c("purple", "red", "blue", "green")) +
  facet_wrap(~ Group_R, scales = "free_y") +
  theme_minimal()
```
...
# Convert the data to long format for easier plotting with ggplot
out_long <- out %>%
  pivot_longer(cols = c("S", "S_A", "S_B", "S_AB", "I", "I_A", "I_B", "I_AB", "R", "R_A", "R_B", "R_AB"), 
               names_to = "Group", values_to = "Individuals")

# Define a larger set of colors
my_colors <- c("purple", "red", "blue", "green", "orange", "pink", "brown", "gray", "cyan", "magenta", "yellow", "black")

# Plot for every possible scenario using facets
ggplot(out_long, aes(x = time, y = Individuals, color = Group)) +
  geom_line() +
  labs(title = "Disease Spread with Vaccination and Culling",
       x = "Time",
       y = "Individuals") +
